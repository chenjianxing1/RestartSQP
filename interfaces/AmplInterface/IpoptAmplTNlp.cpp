...
bool IpoptAmplTNlp::get_starting_point(
    ...
    bool init_constraint_multipliers, double* constraint_multipliers)
{
  ...
  if (init_primal_variables) {
    for (int i = 0; i < num_variables; ++i) {
      primal_variables[i] =
          havex0[i] ? X0[i] : max(LUv[2 * i], min(LUv[2 * i + 1], 0.0));
    }
  }

  if (init_bound_multipliers) {
    // Modified for warm-start from AMPL
    const double* bound_mult = suffix_handler_->GetNumberSuffixValues(
        "bound_mult", AmplSuffixHandler::Variable_Source);
    if (bound_mult) {
      for (int i = 0; i < num_variables; ++i) {
...
void IpoptAmplTNlp::finalize_solution(
    ...
    std::shared_ptr<const Statistics> stats)
{
  ...
  // Copy the bound multipliers into the array that is given to the ASL,
  // potentially adjusting the sign
  if (obj_sign == 1) {
    for (int i = 0; i < num_variables; ++i) {
  ...
  // Copy the activities into the ASL array and give them to the ASL, as long as
  // they are available
...
{
  const double* penalty_parameter = suffix_handler_->GetNumberSuffixValues(
      "penalty_parameter", AmplSuffixHandler::Objective_Source);
...
bool IpoptAmplTNlp::get_initial_working_sets(
    ...
    ActivityStatus* constraints_working_set)
{
  ...
  const int* bounds_activities = suffix_handler_->GetIntegerSuffixValues(
      "activity", AmplSuffixHandler::Variable_Source);
  const int* constraints_activities = suffix_handler_->GetIntegerSuffixValues(
      "activity", AmplSuffixHandler::Constraint_Source);
...
{
  const double* asl_penalty_parameter = suffix_handler_->GetNumberSuffixValues(
      "penalty_parameter", AmplSuffixHandler::Objective_Source);
