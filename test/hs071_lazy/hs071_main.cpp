...
int main(int argv, char* argc[])
{
  ...
  // Create an array with the indices of the constraints that should be
  // considered in the beginning.  Constraints are counted starting from 0.
  ...
  int* constraint_indices =
      new int[num_initial_constraints]; // allocating size 1 just for
                                        // demonstration
  ...
  // constraints.
  lazy_sqp_solver.optimize_nlp(sqp_nlp, num_initial_constraints,
                               constraint_indices);

  // At the end, the finalize_solution method for the original Ipopt NLP should
  // be called. If not, we still need to implement this.
