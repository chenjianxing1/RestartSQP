...
Bool eval_obj(int num_variables, double* primal_variables,
              Bool new_primal_variables, double* objective_value,
              UserDataPtr user_data);

Bool eval_obj_grad(int num_variables, double* primal_variables,
                   Bool new_priml_variables, double* objective_gradient,
                   UserDataPtr user_data);

Bool eval_constr(int num_variables, double* primal_variables,
                 Bool new_primal_variables, int num_constraints,
                 double* constraint_values, UserDataPtr user_data);

Bool eval_constr_jac(int num_variables, double* primal_variables,
                     Bool new_primal_variables, int num_constraints,
                     int num_nonzeros_jacobian, int* row_indices,
                     int* column_indices, double* nonzero_values,
                     UserDataPtr user_data);

Bool eval_lag_hess(int num_variables, double* primal_variables,
                   Bool new_primal_variables, double objective_scaling_factor,
                   int num_constraints, double* constraint_multipliers,
                   Bool new_constraint_multipliers, int num_nonzeros_hessian,
                   int* row_indices, int* column_indices,
...
int main()
{
  ...
  int num_nonzeros_jacobian; /* number of nonzeros in the Jacobian of the
                                constraints */
  int num_nonzeros_hessian;  /* number of nonzeros in the Hessian of the
                                Lagrangian (lower or upper triangular part only)
                              */
  ...
  sqp_problem = CreateSqpProblem(
      num_variables, variable_lower_bounds, variable_upper_bounds,
      num_constraints, constraint_lower_bounds, constraint_upper_bounds,
      num_nonzeros_jacobian, num_nonzeros_hessian, &eval_obj, &eval_obj_grad,
      &eval_constr, &eval_constr_jac, &eval_lag_hess);
              ...
              UserDataPtr user_data)
{
  ...
  *objective_value =
      primal_variables[0] * primal_variables[3] *
          (primal_variables[0] + primal_variables[1] + primal_variables[2]) +
      primal_variables[2];
                 ...
                 double* constraint_values, UserDataPtr user_data)
{
  ...
  constraint_values[0] = primal_variables[0] * primal_variables[1] *
                             primal_variables[2] * primal_variables[3] +
                         my_data->g_offset[0];
  constraint_values[1] = primal_variables[0] * primal_variables[0] +
                         primal_variables[1] * primal_variables[1] +
                         primal_variables[2] * primal_variables[2] +
                         primal_variables[3] * primal_variables[3] +
                         my_data->g_offset[1];
                     ...
                     UserDataPtr user_data)
{
  ...
  if (nonzero_values == NULL) {
    ...
    nonzero_values[0] = primal_variables[1] * primal_variables[2] *
                        primal_variables[3]; /* 0,0 */
    nonzero_values[1] = primal_variables[0] * primal_variables[2] *
                        primal_variables[3]; /* 0,1 */
    nonzero_values[2] = primal_variables[0] * primal_variables[1] *
                        primal_variables[3]; /* 0,2 */
    nonzero_values[3] = primal_variables[0] * primal_variables[1] *
                        primal_variables[2]; /* 0,3 */
                   ...
                   double* nonzero_values, UserDataPtr user_data)
{
  ...
  if (nonzero_values == NULL) {
    ...
    nonzero_values[0] =
        objective_scaling_factor * (2 * primal_variables[3]); /* 0,0 */

    nonzero_values[1] =
        objective_scaling_factor * (primal_variables[3]); /* 1,0 */
    nonzero_values[2] = 0;                                /* 1,1 */

    nonzero_values[3] =
        objective_scaling_factor * (primal_variables[3]); /* 2,0 */
    nonzero_values[4] = 0;                                /* 2,1 */
    nonzero_values[5] = 0;                                /* 2,2 */

    nonzero_values[6] = objective_scaling_factor *
                        (2 * primal_variables[0] + primal_variables[1] +
                         primal_variables[2]); /* 3,0 */
    nonzero_values[7] =
        objective_scaling_factor * (primal_variables[0]); /* 3,1 */
    nonzero_values[8] =
        objective_scaling_factor * (primal_variables[0]); /* 3,2 */
    nonzero_values[9] = 0;                                /* 3,3 */

    /* add the portion for the first constraint */
    nonzero_values[1] -= constraint_multipliers[0] *
                         (primal_variables[2] * primal_variables[3]); /* 1,0 */

    nonzero_values[3] -= constraint_multipliers[0] *
                         (primal_variables[1] * primal_variables[3]); /* 2,0 */
    nonzero_values[4] -= constraint_multipliers[0] *
                         (primal_variables[0] * primal_variables[3]); /* 2,1 */

    nonzero_values[6] -= constraint_multipliers[0] *
                         (primal_variables[1] * primal_variables[2]); /* 3,0 */
    nonzero_values[7] -= constraint_multipliers[0] *
                         (primal_variables[0] * primal_variables[2]); /* 3,1 */
    nonzero_values[8] -= constraint_multipliers[0] *
                         (primal_variables[0] * primal_variables[1]); /* 3,2 */
