...
namespace SQPhotstart {
	...
	public:
		...
		/**
		 * @name Get the optimal solution from the
		 * QPhandler_interface
		 *
		 * This is only an interface for user to avoid call
		 * interface directly.
		 * @param p_k 	the pointer to an empty array with
		 * the length equal to the size of the QP subproblem
		 */
		virtual bool GetOptimalSolution(double* p_k);


		/**
		 *@name Get the multipliers from the QPhandler_interface
		 *
		 * This is only an interface for user to avoid call
	 * interface directly.
		 *
	 ...
	 * subproblem
		...
		/**
		 * @name This function initializes all objects
	 * will be used in this class.
		 *
		 * @param nlp_info
		 * 		it contains the information about number of
	 * 		variables, number of constraints, number of
		 * 		elements in the Hessian and that of Jacobian.
	 * 		The definition of Index_info is in Types.hpp
		 * @param Constraint_type
		 * 		it specifies how the variables are bounded.
	 * 		Please check @ClassifyConstraintType in
	 * 		Algorithm.hpp
		 * 		for more details
		 * @param qptype
		 * 		it specifies which type of QP is going to be
	 * 		solved. It can be either LP, or QP, or SOC
		 */
		virtual bool init(Index_info nlp_info, QPType qptype);

		/**
		 *
		 * @name setup the bounds for the QP subproblems
	 * according to the information from current iterate
		...
		/**
		 * @name This function sets up the object vector g
	 * of the QP problem
		 *
	 ...
	 * at x_k and lambda_k from nlp readers.
		...
		*/
		...
		/** @name setup the matrix A for the QP
	 * subproblems according to the information from current 
	 * iterate
	 * */
		...
		/**
		 * @name solve the QP subproblem according to the
	 * bounds setup before, assuming the first QP
		...
		/**
		 * @name This function copies the information from
	 * another QPhandler objects.
		 * By default, it will copy the vectors of bounds
	 * matrix A information
		 *
		 * If qptype ==LP, then it will not copy the
	 * Hessian, and the first half of the g)
		 * objects in qpOASES
		 */
		virtual bool
		copy_QP_info(
				shared_ptr<const QPhandler> rhs,// the QPhandler object that are going to be copied from
				QPType qptype) { return false; };                // is the object rhs an LP?

		/**
		 * @name This function updates the bounds on x if
	 * there is any changes to the values of trust-region
	 * radius
		 *
		 * @param delta 	 trust region radius
		...
		/**
		 * @name This function updates the vector g in the
	 ...
	 * of gradient in NLP
		...
		/*  @name Update the SparseMatrix H of the QP
	 *  problems when there is any change to the
		...
		/**
		 * @name Update the Matrix H of the QP problems
	 * when there is any change to the
