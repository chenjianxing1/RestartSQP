 ...
 *
 * This C interface is derived from the file IpCInterface.h from the Ipopt
 * project
 */
...
#include "restartsqp/ReturnCodes.h"
  ...
  /** Structure collecting all information about the problem definition and
   * solve statistics etc. */
...
#define FALSE (0)
  ...
  /** A pointer for anything that is to be passed between the called and
   * individual callback function */
  typedef void* UserDataPtr;

  /** Type defining the callback function for evaluating the value of the
   * objective function.
   *
   *  Return value should be set to false if there was a problem doing the
   * evaluation.
   *
   *  See also CrossoverSqpTNlp::eval_objective_value.
   */
  typedef Bool (*Eval_Obj_CB)(int num_variables, double* primal_variables,
                              Bool new_primal_variables,
                              double* objective_value, UserDataPtr user_data);
   ...
   */
  typedef Bool (*Eval_Obj_Grad_CB)(int num_variables, double* primal_variables,
   ...
   */
  typedef Bool (*Eval_Constr_CB)(int num_variables, double* primal_variables,
                                 Bool new_primal_variables, int num_constraints,
                                 double* constraint_values,
                                 UserDataPtr user_data);

  /** Type defining the callback function for evaluating the Jacobian of the
   * constrant functions. Counting of row and column indices starts at 0.
   *
   *  Return value should be set to false if there was a problem doing the
   * evaluation.
   *
  ...
  typedef Bool (*Eval_Constr_Jac_CB)(
      int num_variables, double* primal_variables, Bool new_primal_variables,
      int num_constraints, int num_nonzeros_jacobian, int* row_indices,
      int* column_indices, double* nonzero_values, UserDataPtr user_data);
  ...
  typedef Bool (*Eval_Lag_Hess_CB)(
      int num_variables, double* primal_variables, Bool new_primal_variables,
      double objective_scaling_factor, int num_constraints,
      double* constraint_multipliers, Bool new_constraint_multipliers,
      int num_nonzeros_hessian, int* row_indices, int* column_indices,
      double* nonzero_values, UserDataPtr user_data);

  /** Function for creating a new SQP Problem object.
   *
   *  This function returns an object that can be passed to the
   * CrossoverSqpSolve call. It contains the basic definition of the
   * optimization problem, such as number of variables and constraints, bounds
   * on variables and constraints, information about the derivatives, and the
   * callback function for the computation of the optimization problem functions
   * and derivatives.  During this call, the options file sqp.opt is read as
   * well.
   *
   *  If NULL is returned, there was a problem with one of the inputs
   *  or reading the options file.
   *
   *  See also CrossoverSqpTNlp::get_nlp_info and
   * CrossoverSqpTNlp::get_bounds_info.
   */
  SqpProblem CreateSqpProblem(
      ...
      double* variable_lower_bounds,   /**< Lower bounds on variables
                                        *
                                        * This array of size n is copied
                                        * internally, so that the   caller can
                                        * change the incoming data after   return
                                        * without that SqpProblem is modified.
                                        */
      ...
      double* constraint_lower_bounds, /**< Lower bounds on constraints
                                        *
                                        * This array of size m is copied
                                        * internally, so that the caller can
                                        * change the incoming data after return
                                        * without that SqpProblem is modified.
                                        */
      double* constraint_upper_bounds, /**< Upper bounds on constraints
                                        ...
                                        * internally, so that the caller can
                                        * change the incoming data after return
                                        * without that IpoptProblem is modified.
                                        */
      int num_nonzeros_jacobian, /**< Number of non-zero elements in constraint
                                    Jacobian */
      int num_nonzeros_hessian,  /**< Number of non-zero elements in Hessian of
                                    Lagrangian */
      Eval_Obj_CB
          eval_obj, /**< Callback function for evaluating objective function */
      Eval_Obj_Grad_CB eval_obj_grad,     /**< Callback function for evaluating
                                             gradient of objective function */
      Eval_Constr_CB eval_constr,         /**< Callback function for evaluating
                                             constraint functions */
      Eval_Constr_Jac_CB eval_constr_jac, /**< Callback function for evaluating
                                             Jacobian of constraint functions */
      Eval_Lag_Hess_CB eval_lag_hess      /**< Callback function for evaluating
   ...
   */
  void FreeSqpProblem(SqpProblem sqp_problem);

  /** Function for adding a string option.
   *
   * @return FALSE, if the option could not be set (e.g., if keyword is unknown)
   */
  Bool AddSqpStrOption(SqpProblem sqp_problem, char* keyword, char* val);

  /** Function for adding a Number option.
   *
   * @return FALSE, if the option could not be set (e.g., if keyword is unknown)
   */
  Bool AddSqpNumOption(SqpProblem sqp_problem, char* keyword, double val);

  /** Function for adding an Int option.
   *
   * @return FALSE, if the option  could not be set (e.g., if keyword is
   unknown)
   @*/
  Bool AddSqpIntOption(SqpProblem sqp_problem, char* keyword, int val);
   ...
   */
  Bool OpenSqpOutputFile(SqpProblem sqp_problem, char* file_name,
                         int print_level);
  ...
  enum SqpSolverExitStatus SqpSolve(
      SqpProblem sqp_problem,    /**< Problem that is to be optimized.
                                  *
                                  * Sqp will use the options previously specified
                                  * with    AddSqpOption (etc) for this problem.
                                  */
      double* primal_variables,  /**< Input: Starting point; Output: Optimal
                                    solution */
      double* constraint_values, /**< Values of constraint at final point
                                    (output only; ignored if set to NULL) */
      double* objective_value,   /**< Final value of objective function (output
                                    only; ignored if set to NULL) */
      double*
          bound_multipliers, /**< Input: Initial values for the multipliers for
                              * lower variable bounds (only if warm start option
                              * is chosen); Output: Final multipliers for lower
                              * variable bounds (ignored if set to NULL)
                              */
      double*
          constraint_multipliers, /**< Input: Initial values for the constraint
                                   * multipliers (only if warm start option is
                                   * chosen); Output: Final multipliers for
                                   * constraints (ignored if set to NULL)
                                   */
      char*
          bound_activities, /**< Input: Initial values for the multipliers for
                             * upper variable bounds (only if warm start option
                             * is chosen); Output: Final multipliers for upper
                             * variable bounds (ignored if set to NULL)
                             */
