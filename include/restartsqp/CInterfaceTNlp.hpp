 ...
 *
 * This file is derived from the file IpStdInterfaceNLP.hpp from the Ipopt
 * project
 */
...
public:
   ...
   */
  CInterfaceTNlp(int num_variables, const double* variable_lower_bounds,
                 ...
                 Eval_Obj_CB eval_obj, Eval_Obj_Grad_CB eval_obj_grad,
                 Eval_Constr_CB eval_constr, Eval_Constr_Jac_CB eval_constr_jac,
                 Eval_Lag_Hess_CB eval_lag_hess, double* primal_variables,
   ...
   *
   * These methods are overloaded from TNLP. See TNLP for their more detailed
   * documentation.
   */
  //@{
  ...
  bool get_bounds_info(int num_variables, double* variable_lower_bounds,
...
private:
  ...
  /** Pointer to double array containing starting values for variable
   * multipliers */
  const double* initial_bound_multipliers_;
  /** Pointer to double array containing starting values for constraint
   * multipliers */
  const double* initial_constraint_multipliers_;
  /** Pointer to char array contining the initial activities for variable
   * bounds. */
  ...
  /** A non-const copy of primal variables - this is kept up-to-date in
   * apply_new_primal_variables */
  ...
  void apply_new_primal_variable_(bool new_primal_variables, int num_variables,
                                  const double* primal_variables);
  ...
  /** Copy Constructor */
  CInterfaceTNlp(const CInterfaceTNlp&);

  /** Default Assignment Operator */
  void operator=(const CInterfaceTNlp&);
  //@}
};
