...
CInterfaceTNlp::CInterfaceTNlp(
    int num_variables, const double* variable_lower_bounds,
    const double* variable_upper_bounds, int num_constraints,
    const double* constraint_lower_bounds,
    const double* constraint_upper_bounds, int num_nonzeros_jacobian,
    int num_nonzeros_hessian, const double* initial_primal_variables,
    const double* initial_bound_multipliers,
    const double* initial_constraint_multipliers,
    const char* initial_bound_activities,
    const char* initial_constraint_activities, Eval_Obj_CB eval_obj,
    ...
    double* primal_variables, double* bound_multipliers,
    double* constraint_multipliers, double* constraint_values,
    double* objective_value, char* bound_activities,
...
bool CInterfaceTNlp::get_nlp_info(int& num_variables, int& num_constraints,
...
bool CInterfaceTNlp::get_bounds_info(int num_variables,
                                     ...
                                     double* constraint_upper_bounds)
{
  ...
  std::copy(variable_lower_bounds_, variable_lower_bounds_ + num_variables,
            variable_lower_bounds);
  std::copy(variable_upper_bounds_, variable_upper_bounds_ + num_variables,
    ...
    bool init_constraint_multipliers, double* constraint_multipliers)
{
  ...
  if (init_primal_variables) {
    std::copy(initial_primal_variables_,
              initial_primal_variables_ + num_variables, primal_variables);
  }
  ...
  if (init_bound_multipliers) {
    ...
    std::copy(initial_bound_multipliers_,
              initial_bound_multipliers_ + num_variables, bound_multipliers);
  }
                                          ...
                                          double& objective_value)
{
  ...
  apply_new_primal_variable_(new_primal_variables, num_variables,
                             primal_variables);

  Bool retval =
      (*eval_obj_)(num_variables, non_const_primal_variables_,
                   (Bool)new_primal_variables, &objective_value, user_data_);
                                             ...
                                             double* objective_gradient)
{
  ...
  Bool retval = (*eval_obj_grad_)(num_variables, non_const_primal_variables_,
                                  (Bool)new_primal_variables,
                                  objective_gradient, user_data_);
                                            ...
                                            double* constraint_values)
{
  ...
  Bool retval = (*eval_constr_)(num_variables, non_const_primal_variables_,
                                (Bool)new_primal_variables, num_constraints,
                                constraint_values, user_data_);
    ...
    int* row_indices, int* column_indices, double* nonzero_values)
{
  ...
  assert((row_indices != NULL && column_indices != NULL &&
          nonzero_values == NULL) ||
         (row_indices == NULL && column_indices == NULL &&
          nonzero_values != NULL));

  apply_new_primal_variable_(new_primal_variables, num_variables,
                             primal_variables);
  Bool retval = (*eval_constr_jac_)(num_variables, non_const_primal_variables_,
                                    (Bool)new_primal_variables, num_constraints,
                                    num_nonzeros_jacobian, row_indices,
                                    column_indices, nonzero_values, user_data_);
    ...
    int* column_indices, double* nonzero_values)
{
  ...
  Bool retval = (*eval_lag_hess_)(
      num_variables, non_const_primal_variables_, (Bool)new_primal_variables,
      objective_scaling_factor, num_constraints, non_const_constr_mult,
      (Bool)new_constraint_multipliers, num_nonzeros_hessian, row_indices,
      column_indices, nonzero_values, user_data_);
...
bool CInterfaceTNlp::get_initial_working_sets(
    ...
    ActivityStatus* constraints_working_set)
{
  ...
  if (initial_bound_activities_ == NULL ||
      (num_constraints > 0 && initial_constraint_activities_ == NULL)) {
    return false;
  }
  ...
  for (int i = 0; i < num_constraints; ++i) {
    constraints_working_set[i] =
        (ActivityStatus)initial_constraint_activities_[i];
  }
    ...
    std::shared_ptr<const Statistics> stats)
{
  ...
  if (primal_variables_) {
    std::copy(primal_solution, primal_solution + num_variables,
              primal_variables_);
  }
  ...
  // don't need to store the status, we get the status from the OptimizeTNLP
  // method
}

void CInterfaceTNlp::apply_new_primal_variable_(bool new_primal_variables,
                                                int num_variables,
                                                const double* primal_variables)
{
