...
/** Constructor*/
CplexInterface::CplexInterface(NLPInfo nlp_info, QPType qptype,
...
void CplexInterface::optimizeQP(shared_ptr<Stats> stats)
{
  ...
  if (cplex.getStatus() != IloAlgorithm::Optimal) {
    //        THROW_EXCEPTION(CPLEX_SOLVER_FAILS,"CPLEX does not solve the QP to
    //        optimal");
  }
...
void CplexInterface::optimizeLP(shared_ptr<Stats> stats)
{
  ...
  cplex.solve();
  if (cplex.getStatus() != IloAlgorithm::Optimal) {
    THROW_EXCEPTION(CPLEX_SOLVER_FAILS,
                    "CPLEX does not solve the QP to optimal");
  }
...
void CplexInterface::set_lbA(int location, double value)
{
  ...
  constraints_.at(location) +=
      -1.0 * cplex_vars_[A_->ColNum() + nConstr_QP_ + location];
  constraints_.at(location) -= value;
  for (int i = 0; i < A_->EntryNum(); i++) {
    if (A_->RowIndex(i) == location + 1) {
      constraints_.at(location) +=
          A_->MatVal(i) * cplex_vars_[A_->ColIndex(i) - 1];
    }
  }
  ...
  for (int i = 0; i < rhs->EntryNum(); i++) {
    if (rhs->isSymmetric()) {
      if (rhs->ColIndex(i) == rhs->RowIndex(i)) {
        qobj_ += 0.5 * rhs->MatVal(i) * cplex_vars_[rhs->ColIndex(i) - 1] *
...
void CplexInterface::set_A_values(shared_ptr<const SpTripletMat> rhs,
                                  IdentityInfo I_info)
{
  A_ = rhs;
  I_info_ = I_info;
