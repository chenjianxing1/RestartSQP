...
SqpProblem CreateSqpProblem(
    int num_variables, double* variable_lower_bounds,
    double* variable_upper_bounds, int num_constraints,
    double* constraint_lower_bounds, double* constraint_upper_bounds,
    int num_nonzeros_jacobian, int num_nonzeros_hessian, Eval_Obj_CB eval_obj,
    Eval_Obj_Grad_CB eval_obj_grad, Eval_Constr_CB eval_constr,
    Eval_Constr_Jac_CB eval_constr_jac, Eval_Lag_Hess_CB eval_lag_hess)
{
#if 0
  NEED TO UPDATE
...
{
  return (Bool)sqp_problem->sqp_solver->get_options_list()->SetStringValue(
      keyword, val);
}

Bool AddSqpNumOption(SqpProblem sqp_problem, char* keyword, double val)
{
  return (Bool)sqp_problem->sqp_solver->get_options_list()->SetNumericValue(
      keyword, val);
}

Bool AddSqpIntOption(SqpProblem sqp_problem, char* keyword, int val)
{
  return (Bool)sqp_problem->sqp_solver->get_options_list()->SetIntegerValue(
      keyword, val);
}

Bool SetOutputFileName(SqpProblem sqp_problem, char* file_name)
{
...
void allocate_initial_arrays_(
    SqpProblem sqp_problem, double* primal_variables, double* constraint_values,
    double* objective_value, double* bound_multipliers,
    double* constraint_multipliers, char* bound_activities,
    char* constraint_activities, UserDataPtr user_data,
    double*& initial_primal_variables, double*& initial_bound_multipliers,
    double*& initial_constraint_multipliers, char*& initial_bound_activities,
    char*& initial_constraint_activities, shared_ptr<CInterfaceTNlp>& sqp_tnlp)
{
  ...
  sqp_tnlp = make_shared<CInterfaceTNlp>(
      num_variables, sqp_problem->variable_lower_bounds,
      sqp_problem->variable_upper_bounds, num_constraints,
      sqp_problem->constraint_lower_bounds,
      sqp_problem->constraint_upper_bounds, sqp_problem->num_nonzeros_jacobian,
      sqp_problem->num_nonzeros_hessian, initial_primal_variables,
      initial_bound_multipliers, initial_constraint_multipliers,
      initial_bound_activities, initial_constraint_activities,
      sqp_problem->eval_obj, sqp_problem->eval_obj_grad,
      sqp_problem->eval_constr, sqp_problem->eval_constr_jac,
      sqp_problem->eval_lag_hess, primal_variables, bound_multipliers,
      constraint_multipliers, constraint_values, objective_value,
      bound_activities, constraint_activities, user_data);
}

SqpSolverExitStatus SqpSolve(SqpProblem sqp_problem, double* primal_variables,
                             double* constraint_values, double* objective_value,
...
{
  if (!primal_variables) {
    sqp_problem->sqp_solver->get_jnlst()->Printf(
        J_ERROR, J_MAIN,
  ...
  allocate_initial_arrays_(
      sqp_problem, primal_variables, constraint_values, objective_value,
      bound_multipliers, constraint_multipliers, bound_activities,
      constraint_activities, user_data, initial_primal_variables,
      initial_bound_multipliers, initial_constraint_multipliers,
      initial_bound_activities, initial_constraint_activities, sqp_tnlp);
...
SqpSolverExitStatus
CrossoverSolve(SqpProblem sqp_problem, double* primal_variables,
               double* constraint_values, double* objective_value,
               double* bound_multipliers, double* constraint_multipliers,
               char* bound_activities, char* constraint_activities,
...
SqpSolverExitStatus
SqpResolve(SqpProblem sqp_problem, double* primal_variables,
           double* constraint_values, double* objective_value,
           double* bound_multipliers, double* constraint_multipliers,
           char* bound_activities, char* constraint_activities,
           UserDataPtr user_data)
