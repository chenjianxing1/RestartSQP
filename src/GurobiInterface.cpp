 ...
 * Date:2019-09-06
 */
#include "restartsqp/GurobiInterface.hpp"

namespace SQPhotstart {

GurobiInterface::GurobiInterface(NLPInfo nlp_info, QPType qptype,
...
void GurobiInterface::optimizeQP(shared_ptr<Stats> stats)
{
  ...
  if (grb_mod_->get(GRB_IntAttr_Status) != GRB_OPTIMAL) {
    printf("qp is not optimal, the current status is %i",
           grb_mod_->get(GRB_IntAttr_Status));
  }
  for (int i = 0; i < nConstr_QP_ * 2; i++) {
    if (grb_mod_->getConstr(i).get(GRB_DoubleAttr_Pi) > 0)
      y_qp->set_value((int)i / 2,
                      grb_mod_->getConstr(i).get(GRB_DoubleAttr_Pi));
 ...
 *
 */
void GurobiInterface::get_working_set(ActiveType* W_constr,
                                      ActiveType* W_bounds)
{
}
...
void GurobiInterface::set_lbA(int location, double value)
{
  ...
  grb_mod_->addConstr(linlhs, GRB_GREATER_EQUAL, 0,
                      "lbA_" + to_string(location));
}
  ...
  for (int i = 0; i < rhs->EntryNum(); i++) {
    if (rhs->isSymmetric()) {
      if (rhs->ColIndex(i) == rhs->RowIndex(i)) {
        qobj_ += 0.5 * rhs->MatVal(i) * grb_vars_[rhs->ColIndex(i) - 1] *
...
void GurobiInterface::set_A_values(shared_ptr<const SpTripletMat> rhs,
                                   IdentityInfo I_info)
{
